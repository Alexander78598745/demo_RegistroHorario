...
class RegistroHorarioApp {
    constructor() {
        ...
        this.currentScreen = 'mainContent'; // DEMO: Siempre disponible
    ...
    init() {
        ...
        setTimeout(() => {
            ...
            this.showDemoMode();
            // Forzar pantalla principal si por alguna razón no está visible
            if (this.currentScreen !== 'mainContent') {
    ...
    renderPendingDaysBeforeStart() {
        ...
        container.innerHTML = `
            <div class="pre-start-message" style="text-align: center; padding: 40px 20px; color: #64748b;">
                <i class="fas fa-calendar-plus" style="font-size: 3rem; color: #3b82f6; margin-bottom: 20px; display: block;"></i>
    ...
    async handleFormSubmit(e) {
        ...
        try {
            ...
            const emailStatus = navigator.onLine ? 
                'PDF generado y enviado por email.' : 
    ...
    async sendEmailNow(emailData) {
        ...
        console.log('DEMO: Simulando envío de email a:', emailData.destinatario);
        await new Promise(resolve => setTimeout(resolve, 1000));
    }

    saveEmailForLater(emailData) {
        const pendingEmails = JSON.parse(localStorage.getItem('pending_emails_demo') || '[]');
        ...
        this.blobToBase64(emailData.pdfBlob).then(base64 => {
            ...
            localStorage.setItem('pending_emails_demo', JSON.stringify(pendingEmails));
            
            console.log('DEMO: Email guardado para envío posterior');
        });
    }
    ...
    async processPendingEmails() {
        ...
        console.log(`DEMO: Procesando ${pendingEmails.length} emails pendientes...`);
